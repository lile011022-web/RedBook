...
素材库按账号隔离。账号 A 只能看到账号 A 的素材，账号 B 只能看到账号 B 的素材。
...
- 输入素材目标
- AI 会给出封面画面、拍摄脚本、视频分镜和素材清单

如果同一个文件被不同账号重复上传，系统会记录跨账号复用风险提醒。
...
- 选择账号，确保该账号已维护 `人设`
- 选择生成数量，支持 1 到 5 版
- 可填写补充要求，例如 `更口语化`、`偏直播招聘`、`标题更自然`
- 不填写补充要求也可以直接点击 `生成文案`
- 生成后默认是 `needs_review`
- 对不满意的文案，可以再次点击 `生成文案` 追加新的候选草稿
- 必须人工点击 `通过` 后才能排期
